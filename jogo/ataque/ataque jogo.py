...
CODE_WORDS = [
    "if", "else", "while", "for", "def", "class", "return", "break", "import",
    "try", "except", "finally", "print", "input", "int", "str", "bool",
    "True", "False", "None", "with", "open", "range", "len", "append",
    "self", "__init__", "super", "global", "lambda", "yield", "map", "filter",
    "async", "await", "continue", "del", "raise", "assert", "enumerate",
    "zip", "sorted", "reversed", "isinstance", "getattr", "setattr",
    "pass", "not", "and", "or"
]

# Paleta defensiva (preto e vermelho)
DEF_PRIMARY   = "#1A0000"   # vermelho escuro profundo (quase preto)
DEF_SECONDARY = "#8B0000"   # vermelho escuro
DEF_ACCENT    = "#FFCCCC"   # vermelho claro para acento
BG_OVERLAY    = "#0D0D0D"   # fundo escuro com leve transparência (preto suave)

# Paleta do menu (foco em legibilidade no vermelho)
MENU_TEXT        = "#FFEAEA"  # texto principal (alto contraste com fundo escuro)
MENU_SHADOW      = "#330000"  # sombra/glow do título em tom vermelho escuro
MENU_CARD_BG     = "#1A0A0A"  # cartão central (preto com vermelho escuro)
MENU_ENTRY_BG    = "#330D0D"  # campo de entrada (escuro com leve tom vermelho)
MENU_BTN_BG      = "#B22222"  # botão normal (vermelho tijolo)
MENU_BTN_BG_HOV  = "#FF0000"  # botão hover (vermelho vibrante)
MENU_ACCENT_SOFT = "#FF6666"  # acento suave (vermelho claro)

BASE_DIR = os.path.dirname(__file__)
HIGHSCORE_FILE = os.path.join(BASE_DIR, "highscore.json")
MENU_BG_CANDIDATES = [
os.path.join(BASE_DIR, "ataque-mapa.png"),  # recomendado: mapa-múndi azul
    os.path.join(BASE_DIR, "gradiente-vermelho.png")
]
...
class BaseGame:
    ...
    def show_start_screen(self):
        ...
        title_canvas.pack()
        title_text = "     HACKER CHALLENGE - ATAQUE"
        # sombra
    ...
    def load_background_images(self):
        base_dir = BASE_DIR
        try:
            world = Image.open(os.path.join(base_dir, "ataque-mapa.png"))
        except Exception:
            world = None
        try:
            texture = Image.open(os.path.join(base_dir, "gradiente-vermelho.png"))
        except Exception:
            texture = None
    ...
    def draw_decorations(self):
        ...
        x = w - 120
        self.canvas.create_oval(x-50, 120-50, x+50, 120+50, outline=DEF_PRIMARY, width=4, tags=("decor",))
        self.canvas.create_text(x, 120, text="🛡️", font=("Consolas", 28), tags=("decor",))
        for i in range(6):
            self.canvas.create_line(30, 150 + i*20, 200, 130 + i*18, fill=DEF_PRIMARY, width=1, dash=(3,4), tags=("decor",))
